...
> Note: These are the bare minimum needed for ProjectKiwi Backend to run, please do not attempt to host a publicly accessible version of ProjectKiwi if you do not know what you are doing

### Optional Services
 - ProjectKiwi Gameserver running on Windows Server ( Needed for rendering and games )
 - ProjectKiwi Discord Bot
...
11. **DISCORD_BOT_AUTHTOKEN** - Authorization Token for ProjectKiwi Discord Bot, use random long string
...
29. **DISCOURSE_SSO_ENABLED** - Allows authentication with ProjectKiwi for [Discourse](https://www.discourse.org/) Forums
...
## KeyPair Generation
The ProjectKiwi Backend requires some keys for it to sign and communicate with gameservers, run the script below to generate those keys.
```
