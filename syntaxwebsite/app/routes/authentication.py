...
def loginNegotiate():
    ...
    if domain:
        resp.set_cookie(".ROBLOSECURITY", newAuthToken, expires=datetime.utcnow() + timedelta(days=365), domain=domain)
    else:
        resp.set_cookie(".ROBLOSECURITY", newAuthToken, expires=datetime.utcnow() + timedelta(days=365))
...
def verifyTwoStep():
    ...
    if Validated2FA:
        ...
        domain = f".{config.BaseDomain}" if getattr(config, 'BaseDomain', '') not in ("localhost", "127.0.0.1", "") else None
        if domain:
            Response.set_cookie(".ROBLOSECURITY", sessionToken, expires=datetime.utcnow() + timedelta(days=365), domain=domain)
        else:
            Response.set_cookie(".ROBLOSECURITY", sessionToken, expires=datetime.utcnow() + timedelta(days=365))
        return Response
