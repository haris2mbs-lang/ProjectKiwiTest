...
def SiteStatsHTTP():
    ...
    BannedByAnticheat = UserBan.query.filter_by(reason="Exploiting in games is not tolerated on ProjectKiwi").count() # meme
...
def LookupUserByDiscordId(discordid : int):
    ...
    if LinkedDiscordObj is None:
        return ReturnError("No ProjectKiwi account is associated with this Discord ID", 404)
