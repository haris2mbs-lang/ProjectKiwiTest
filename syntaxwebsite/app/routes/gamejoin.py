...
def gamejoin_api_v1():
    ...
    resp = make_response(jsonify({
        ...
        "joinScript": {
            ...
            "CharacterAppearance": f"http://www.ProjectKiwi.eco/v1/avatar-fetch?userId={str(AuthenticatedUser.id)}&placeId={str(requestedPlaceId)}",
...
def GenerateClientTicket( UserObj : User, JobId : str, CharacterURL : str = None, CustomTimestamp : str = "", TicketVersion : int = 1, PlaceId : int = 1) -> str:
    ...
    if CharacterURL is None:
        if TicketVersion == 2:
            ...
            CharacterURL = Config.BaseURL + "/Asset/CharacterFetch.ashx?userId=" + str(UserObj.id) # f"http://www.ProjectKiwi.eco/v1.1/avatar-fetch?userId={str(UserObj.id)}&placeId={str(PlaceId)}"
        elif TicketVersion == 4:
            CharacterURL = f"http://www.ProjectKiwi.eco/v1/avatar-fetch?userId={str(UserObj.id)}&placeId={str(PlaceId)}"
