...
def authenticated_client_endpoint(f):
    @wraps(f)
    def decorated_function(*args, **kwargs):
        if request.cookies.get("ProjectKiwi-Session-Id", type = str ) is not None:
            DataSections = request.cookies.get("ProjectKiwi-Session-Id", type = str )
        else:
...
def GetCurrentUser() -> User:
    ...
    def _get_user() -> User:
        if ".ROBLOSECURITY" not in request.cookies:
            if "ProjectKiwi-Session-Id" not in request.cookies:
                return None
            else:
                DataSections = request.cookies["ProjectKiwi-Session-Id"].split("|")
            if len(DataSections) != 9:
                return None
