...
USE_LOCAL_CONFIG = os.environ.get('FLASK_LOCAL_DEV', '').lower() in ('true', '1', 'yes')
if USE_LOCAL_CONFIG:
    from config import LocalConfig as ConfigClass
else:
...
logname = "./logs/ProjectKiwiweb.log"
...
def create_app(config_class=None):
    # Use ConfigClass from module level if no config_class provided
    ...
    # Use filesystem session for local development, Redis for production
    if USE_LOCAL_CONFIG:
        ...
        app.config['SESSION_REDIS'] = redis.from_url(config_class.FLASK_LIMITED_STORAGE_URI)
    ...
    csrf.init_app(app)
    # Disable CSRF in local development for easier testing
    if USE_LOCAL_CONFIG:
    ...
    def after_request( response : Response ):
        ...
        if hasattr(response, 'direct_passthrough') and not response.direct_passthrough:
            ...
            if request.cookies.get(key="t", default=None, type=str) is None:
                ...
                domain = f".{config_class.BaseDomain}" if getattr(config_class, 'BaseDomain', '') not in ("localhost", "127.0.0.1", "") else None
                if domain:
                    response.set_cookie("t", NewToken, expires=datetime.utcnow() + timedelta(days=365), domain=domain)
                else:
                    response.set_cookie("t", NewToken, expires=datetime.utcnow() + timedelta(days=365))
