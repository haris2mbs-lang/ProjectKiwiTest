...
def CreateLoginRecord( UserId : int, TokenOverwrite : str = None):
    ...
    userIPHash = hashlib.sha512(
        string = f"{get_remote_address()}:ProjectKiwi:{config.FLASK_SESSION_KEY}".encode("utf-8")
    ).hexdigest()
    ...
    # Ensure primary key present for DBs that don't auto-populate integer PKs (local sqlite)
    try:
        if newLoginRecord.id is None:
            max_id = db.session.query(func.max(LoginRecord.id)).scalar() or 0
            newLoginRecord.id = int(max_id) + 1
    except Exception:
        pass
...
def login_page():
    ...
    # generate a simple arithmetic captcha for the login page
    try:
    ...
    login_page_response = make_response(render_template("login/login.html", loginEnabled=loginEnabled, captchaQuestion=captcha_question))
    if request.cookies.get( key = ".ROBLOSECURITY", default = None, type = str ) is not None:
        domain = f".{config.BaseDomain}" if getattr(config, 'BaseDomain', '') not in ("localhost", "127.0.0.1", "") else None
        if domain:
...
def handle_login_post():
    ...
    # Validate simple arithmetic captcha (server-side)
    captcha_response = request.form.get(key="captcha", default=None, type=str)
    expected = session.get("login_captcha_answer")
    # remove stored captcha after reading to prevent replay
    try:
        session.pop("login_captcha_answer", None)
    except Exception:
        pass

    if expected is None or captcha_response is None or captcha_response.strip() == "":
...
def totp_validate_post():
    ...
    domain = f".{config.BaseDomain}" if getattr(config, 'BaseDomain', '') not in ("localhost", "127.0.0.1", "") else None
    if domain:
        resp.set_cookie(".ROBLOSECURITY", sessionToken, expires=datetime.utcnow() + timedelta(days=365), domain=domain)
    else:
        resp.set_cookie(".ROBLOSECURITY", sessionToken, expires=datetime.utcnow() + timedelta(days=365))
    del session["totpvalidatesession"]
    if request.cookies.get("t", default=None, type=str) is not None:
        NewToken = ''.join(random.choice(string.ascii_letters + string.digits) for _ in range(128))
        if domain:
            resp.set_cookie("t", NewToken, expires=datetime.utcnow() + timedelta(days=365), domain=domain)
        else:
            resp.set_cookie("t", NewToken, expires=datetime.utcnow() + timedelta(days=365))
        CreateLoginRecord( user.id, NewToken )
    else:
        CreateLoginRecord( user.id )
    return resp
...
def reset_password_post():
            ...
            "Messages": [
                {
                    "From": {
                        ...
                        "Name": "ProjectKiwi.eco"
                    },
                    "To": [
                        {
                            "Email": user_email,
                            "Name": "ProjectKiwi User"
                        }
                    ],
                    "TemplateID": Config.MAILJET_PASSWORDRESET_TEMPLATE_ID,
                    "TemplateLanguage": True,
                    "Subject": f"ProjectKiwi - Reset Password",
                    "Variables": {
    ...
    flash("A reset password email has been sent if the email is linked to a ProjectKiwi account", "success")
    return redirect("/reset-password")
