...
def LogModerationAction(
        ...
        relatedAssets = [
            {
                ...
                "source": "https://cdn.ProjectKiwi.eco/",
                "page": "https://www.ProjectKiwi.eco/catalog/1/",
                "creator": User | Group
            }
        ]
    """
    ...
    embed = {
        ...
        "author": {
            "name": Actor.username,
            "icon_url": f"https://www.ProjectKiwi.eco/Thumbs/Head.ashx?x=48&y=48&userId={str(Actor.id)}"
        },
        "footer": {
            "text": f"ProjectKiwi Asset Moderation Log"
        },
    ...
    def thread_func():
        try:
            requests.post(
                ...
                json = {
                    "username": "ProjectKiwi Asset Moderation Log",
                    "embeds": [embed],
                    "avatar_url": f"https://www.ProjectKiwi.eco/Thumbs/Head.ashx?x=48&y=48&userId={str(Actor.id)}"
                }
            )
        except Exception as e:
...
def ApprovePendingGroupIcon( content_hash ):
    ...
    for iconObj in GroupIconObj:
        ...
        LogModerationAction(auth.GetCurrentUser(), isApproved=True, relatedAssets=[{
            ...
            "page": f"https://www.ProjectKiwi.eco/groups/{str(iconObj.group.id)}/",
...
def ApprovePendingAsset(assetid):
    ...
    RelatedAssets.append({
        ...
        "source": None,
        "page": f"https://www.ProjectKiwi.eco/catalog/{str(AssetObj.id)}/",
        ...
        if ParentAsset is not None:
            if ParentAsset.moderation_status == 1:
                ...
                RelatedAssets.append({
                    ...
                    "source": None,
                    "page": f"https://www.ProjectKiwi.eco/catalog/{str(ParentAsset.id)}/",
                    "creator": GetCreatorOfAsset(AssetObj)
                })
            # Get the AssetThumbnail
...
def ApprovePendingIcon(assetid):
    ...
    LogModerationAction(auth.GetCurrentUser(), isApproved=True, relatedAssets=[{
        ...
        "page": f"https://www.ProjectKiwi.eco/games/{str(PlaceAssetObj.id)}/",
...
def DeclinePendingIcon(assetid):
    ...
    LogModerationAction(auth.GetCurrentUser(), isApproved=False, relatedAssets=[{
        ...
        "source": f"{config.CDN_URL}/{PlaceIconObj.contenthash}",
        "page": f"https://www.ProjectKiwi.eco/games/{str(PlaceIconObj.placeid)}/",
...
def ApprovePendingThumbnail(thumbnailid : int):
    ...
    LogModerationAction(auth.GetCurrentUser(), isApproved=True, relatedAssets=[{
        ...
        "source": f"{config.CDN_URL}/{AssetThumbnailObj.content_hash}",
        "page": f"https://www.ProjectKiwi.eco/catalog/{str(AssetThumbnailObj.asset_id)}/",
...
def LogUserBanAction( BannedUser : User, Actor : User, BanObject : UserBan ):
    BanEmbed = {
        ...
        "author": {
            ...
            "icon_url": f"https://www.ProjectKiwi.eco/Thumbs/Head.ashx?x=48&y=48&userId={str(BannedUser.id)}"
        },
        "footer": {
            "text": "ProjectKiwi Asset Moderation Log"
        },
        "timestamp": datetime.utcnow().isoformat()
    }
    def thread_func():
        try:
            requests.post(
                url = config.DISCORD_ADMIN_LOGS_WEBHOOK,
                json = {
                    "username": "ProjectKiwi Moderation Log",
                    "embeds": [BanEmbed],
                    "avatar_url": f"https://www.ProjectKiwi.eco/Thumbs/Head.ashx?x=48&y=48&userId={str(BannedUser.id)}"
                }
            )
        except Exception as e:
...
def LogUserUnbanAction( TargetUser : User, Actor : User, BanObj : UserBan ):
    ...
    BanEmbed = {
        ...
        "author": {
            ...
            "icon_url": f"https://www.ProjectKiwi.eco/Thumbs/Head.ashx?x=48&y=48&userId={str(TargetUser.id)}"
        },
        "footer": {
            "text": "ProjectKiwi Asset Moderation Log"
        },
        ...
        try:
            requests.post(
                ...
                json = {
                    "username": "ProjectKiwi Moderation Log",
                    "embeds": [BanEmbed],
                    "avatar_url": f"https://www.ProjectKiwi.eco/Thumbs/Head.ashx?x=48&y=48&userId={str(TargetUser.id)}"
                }
            )
        except Exception as e:
...
def CreateUserPost():
    ...
    try:
        ...
        flash("Username is not friendly for ProjectKiwi")
        return redirect("/signup")
...
def UpdateAssetFilePost():
    ...
    if AssetObj.creator_id not in [1,2] or AssetObj.creator_type != 0:
        flash("Asset is not owned by ProjectKiwi", "error")
        return redirect("/admin/update-asset-file")
...
def LogAssetModerationAction( Actor : User, AssetObj : Asset ):
    def thread_func():
        EmbedObj = {
            ...
            "description": f"Asset [{AssetObj.id}](https://www.ProjectKiwi.eco/admin/moderate-asset/{AssetObj.id}) moderation status has been updated by **{Actor.username}** ({Actor.id})",
...
def LotteryPost():
    ...
    for UserAssetObj in EligibleLimiteds:
        ...
        CreateSystemMessage( 
            ...
            f"""You have been randomly selected to be given a free limited by ProjectKiwi.
...
""",
        ...
        CreateSystemMessage(
            ...
            f""""Your limited {UserAssetObj.asset.name} ( {UserAssetObj.asset.id} ) [ UAID: {UserAssetObj.id} / Serial: { UserAssetObj.serial if UserAssetObj.serial is not None else 'None'} ] has been removed from your account and was given to another ProjectKiwi User
