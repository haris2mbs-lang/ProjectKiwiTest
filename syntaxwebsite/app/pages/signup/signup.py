...
def signup_page():
    ...
    if not websiteFeatures.GetWebsiteFeature("WebSignup"):
        ...
        SignupEnabled = False
    # generate a simple arithmetic captcha for local/dev
    try:
        a = random.randint(1, 9)
        b = random.randint(1, 9)
        session["signup_captcha_answer"] = str(a + b)
        captcha_question = f"{a} + {b} = ?"
    except Exception:
        session.pop("signup_captcha_answer", None)
        captcha_question = None

    return render_template("signup/signup.html", SignupEnabled=SignupEnabled, captchaQuestion=captcha_question)
...
def signup_post():
    ...
    # If a Cloudflare token was provided, verify it. Otherwise the arithmetic captcha was already validated below.
    if cf_token:
