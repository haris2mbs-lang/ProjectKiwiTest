...
fn get_installation_directory() -> PathBuf {
    return PathBuf::from(data_local_dir().unwrap().to_str().unwrap()).join("ProjectKiwi");
}
...
async fn main() {
    ...
    let base_url : &str = "www.ProjectKiwi.eco";
    let mut setup_url : &str = "setup.ProjectKiwi.eco";
    let fallback_setup_url : &str = "d2f3pa9j0u8v6f.cloudfront.net";
    let mut bootstrapper_filename :&str = "ProjectKiwiPlayerLauncher.exe";
    #[cfg(not(target_os = "windows"))]
    {
        bootstrapper_filename = "ProjectKiwiPlayerLinuxLauncher";
    }
   ...
   {} | Build Date: {} | Version: {}", base_url ,build_date, env!("CARGO_PKG_VERSION"));
    ...
    if terminal_width < 80 {
        print!("{}\n", format!("ProjectKiwi Bootstrapper | {} | Build Date: {} | Version: {}", base_url, build_date, env!("CARGO_PKG_VERSION")).to_string().magenta().cyan().italic().on_black()); // Fallback message
    } else {
...
Name=ProjectKiwi Launcher
Exec={} %u
Icon={}
Type=Application
Terminal=true
Version={}
MimeType=x-scheme-handler/ProjectKiwi-player;", latest_bootstrapper_path.to_str().unwrap(), latest_bootstrapper_path.to_str().unwrap(), env!("CARGO_PKG_VERSION"));
                
                let desktop_file_path = dirs::data_local_dir().unwrap().join("applications").join("ProjectKiwi-player.desktop");
                std::fs::write(desktop_file_path, desktop_file_content).unwrap();

                info("Please launch ProjectKiwi from the website, to continue with the update process.");
                std::thread::sleep(std::time::Duration::from_secs(20));
            }
            std::process::exit(0);
    ...
    if !app_settings_path.exists() { //|| !client_executable_path.exists() {
        ...
        // Install the ProjectKiwi-player scheme in the registry
        info("Installing ProjectKiwi-player scheme");
        #[cfg(target_os = "windows")]
        {
            ...
            let hkey_ProjectKiwi_player = hkey_classes_root.create_subkey("ProjectKiwi-player").unwrap().0;
            let hkey_ProjectKiwi_player_shell = hkey_ProjectKiwi_player.create_subkey("shell").unwrap().0;
            let hkey_ProjectKiwi_player_shell_open = hkey_ProjectKiwi_player_shell.create_subkey("open").unwrap().0;
            let hkey_ProjectKiwi_player_shell_open_command = hkey_ProjectKiwi_player_shell_open.create_subkey("command").unwrap().0;
            let defaulticon = hkey_ProjectKiwi_player.create_subkey("DefaultIcon").unwrap().0;
            hkey_ProjectKiwi_player_shell_open_command.set_value("", &format!("\"{}\" \"%1\"", latest_bootstrapper_path.to_str().unwrap())).unwrap();
            defaulticon.set_value("", &format!("\"{}\",0", latest_bootstrapper_path.to_str().unwrap())).unwrap();
            hkey_ProjectKiwi_player.set_value("", &format!("URL: ProjectKiwi Protocol")).unwrap();
            hkey_ProjectKiwi_player.set_value("URL Protocol", &"").unwrap();
        }
        #[cfg(not(target_os = "windows"))]
        {
            // Linux support
...
<Settings>
    ...
    // Looks something like "ProjectKiwi-player://1+launchmode:play+gameinfo:TICKET+placelauncherurl:https://www.ProjectKiwi.eco/Game/placelauncher.ashx?placeId=660&t=TICKET+k:l"
    debug(&format!("Arguments Passed: {}", args.join(" ").bright_blue()));
    if args.len() == 1 {
        // Just open the website
        #[cfg(target_os = "windows")]
        {
            std::process::Command::new("cmd").arg("/c").arg("start").arg("https://www.ProjectKiwi.eco/games").spawn().unwrap();
            std::process::exit(0);
        }
        #[cfg(not(target_os = "windows"))]
        {
            std::process::Command::new("xdg-open").arg("https://www.ProjectKiwi.eco/games").spawn().unwrap();
            std::process::exit(0);
        }
    }

    let main_args = &args[1];
    let main_args = main_args.replace("ProjectKiwi-player://", "");
    ...
    if client_year == "2018" {
        client_executable_path = current_version_directory.join("Client2018").join("ProjectKiwiPlayerBeta.exe");
    } else if client_year == "2020" {
        client_executable_path = current_version_directory.join("Client2020").join("ProjectKiwiPlayerBeta.exe");
    } else if client_year == "2014" {
        client_executable_path = current_version_directory.join("Client2014").join("ProjectKiwiPlayerBeta.exe");
    } else if client_year == "2021" {
        client_executable_path = current_version_directory.join("Client2021").join("ProjectKiwiPlayerBeta.exe");
    } else {
        client_executable_path = current_version_directory.join("Client2016").join("ProjectKiwiPlayerBeta.exe");
    }
    if !client_executable_path.exists() {
        ...
        error("Failed to run ProjectKiwiPlayerBeta.exe, is your antivirus removing it? The bootstrapper will attempt to redownload the client on next launch.");
        std::thread::sleep(std::time::Duration::from_secs(20));
        std::process::exit(0);
    }
    match launch_mode.as_str() {
        "play" => {
            info("Launching ProjectKiwi");
            #[cfg(target_os = "windows")]
            {           
